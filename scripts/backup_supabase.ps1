...
#
# 実行時間帯(2026-09-07変更、2026-09-08再変更): egress(データ転送量)削減のため、
# 1日12回(10:00-21:00毎時)→1日4回(10:00/13:00/16:00/19:00の3時間おき)→
# 1日3回(12:00/16:00/20:00の4時間おき)と段階的に減らした。タスクスケジューラの
# トリガー自体は毎時のままで構わない(このスクリプトが$RunHoursに無い時刻の起動を
# 即終了させる)。トリガー間隔自体を4時間おきに変更したい場合は、このファイル末尾の
...
# 2026-09-08: さらに1日3回(12:00/16:00/20:00の4時間おき)に変更した。

...
#
# 実行時間帯(2026-09-07変更、2026-09-08再変更、2026-09-08さらに変更): egress
# (データ転送量)削減のため、1日12回(10:00-21:00毎時)→1日4回(10:00/13:00/16:00/19:00の
# 3時間おき)→1日3回(12:00/16:00/20:00の4時間おき)→1日2回(16:00/20:00)と段階的に
# 減らした。12:00は日次バックアップ(backup_supabase_daily.ps1)の実行時刻と重複する
# ため除外した。タスクスケジューラのトリガー自体は毎時のままで構わない(このスクリプトが
# $RunHoursに無い時刻の起動を即終了させる)。トリガー間隔自体を変更したい場合は、
# このファイル末尾のコメント「タスクスケジューラのトリガーを変更する場合の手順」を参照。
#
...
# 上記の$RunHours判定により、タスク自体は毎時起動のままでも1日2回(16:00/20:00)しか
# 実際にはバックアップ処理が走らない(それ以外の起動はスキップして即終了する)。その
# ためタスクスケジューラの設定は変更しなくても今回の削減目的は達成できる。
#
# もしタスク自体の起動回数(プロセス起動のオーバーヘッド)も減らしたい場合は、
# 以下の手順でトリガーを毎時起動→16:00/20:00の1日2回起動に変更できる:
#   1. タスクスケジューラ(taskschd.msc)を開く
#   2. タスクスケジューラ ライブラリから "KIC_Supabase_Backup" を探して右クリック→プロパティ
#   3. [トリガー]タブ→既存のトリガーを選択→[編集]
#   4. 開始時刻を16:00に変更し、「詳細設定」内の「繰り返し間隔」を「1 時間」から
#      「4 時間」・「継続時間」を「4 時間」に変更する(16:00起動・4時間おき・4時間だけ
#       継続=16:00と20:00の2回のみ実行される。継続時間を短くしすぎると20:00の実行が
#       トリガーされない点に注意)
#   5. [OK]で保存

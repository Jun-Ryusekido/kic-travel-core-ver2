...
#
# 実行時間帯(2026-09-07変更、2026-09-08再変更、2026-09-08さらに変更、2026-09-12再変更):
# egress(データ転送量)削減のため、1日12回(10:00-21:00毎時)→1日4回
# (10:00/13:00/16:00/19:00の3時間おき)→1日3回(12:00/16:00/20:00の4時間おき)→
# 1日2回(16:00/20:00)→1日1回(18:00)と段階的に減らした。Supabase無料枠のegress
# 残量が逼迫したための緊急対応。12:00は日次バックアップ(backup_supabase_daily.ps1)の
# 実行時刻と重複するため除外した。タスクスケジューラのトリガー自体は毎時のままで構わない
# (このスクリプトが$RunHoursに無い時刻の起動を即終了させる)。トリガー間隔自体を
# 変更したい場合は、このファイル末尾のコメント「タスクスケジューラのトリガーを変更する
# 場合の手順」を参照。
#
...
# ===== 緊急一時停止ガード(2026-09-12追加、2026-09-14に9-14~9-18を追加、2026-09-18に9-19~9-21を追加) =====
# Supabase無料枠のegress残量が枯渇寸前(残り0.9GB)のため、2026-09-12・09-13の2日間は
# バックアップ処理そのものを完全停止する緊急措置を実施した。
# 2026-09-14: egress枯渇が収まっていないため、2026-09-14〜2026-09-18の5日間も追加で
# 完全停止する(9-13以前の状態には手を入れず、9-14〜9-18を追加しただけ)。
# 2026-09-18: 現行の課金期間(8/22-9/22)が終わるまでegressがほぼ回復しないため、
# 期間終了直前の9-19〜9-21も追加で完全停止する(9-22の課金期間リセットで自動的に
# 5GBの枠が回復するため、9-22以降はこのガードに関係なく通常通り動作する)。
# 【重要】この停止期間中はバックアップが一切行われないため、この間にデータ障害が
# 発生した場合、直前の正常なバックアップ時点までしか復旧できないリスクがある。
# あくまで一時的な緊急措置であり、9-22以降はこのガードに関係なく自動的に通常通り
# 動作する(下の$RunHoursによる時刻制限のみが適用される)。
# このブロックは一時的なものであり、9-22以降に削除してよい。
$EmergencyPauseDates = @('2026-09-12', '2026-09-13', '2026-09-14', '2026-09-15', '2026-09-16', '2026-09-17', '2026-09-18', '2026-09-19', '2026-09-20', '2026-09-21')
$todayStr = (Get-Date).ToString('yyyy-MM-dd')
if (-not $SkipTimeWindowCheck -and ($EmergencyPauseDates -contains $todayStr)) {
...
# (10:00/13:00/16:00/19:00)に変更。
# 2026-09-08: 1日3回(12:00/16:00/20:00の4時間おき)に変更。
# 2026-09-08: 12:00は日次バックアップ(backup_supabase_daily.ps1、KIC_Supabase_Backup_Daily
# タスク)の実行時刻と重複するため除外し、1日2回(16:00/20:00)に変更した。
# 2026-09-12: 1日1回(18:00)に変更。12:00の日次バックアップと重複しないようにするため。
# タスクスケジューラのトリガー間隔は変更せず、このスクリプト自身が対象外の時刻を
# 判定して即終了する(従来の10:00-22:00の範囲判定と同じ仕組みを、範囲ではなく
# 特定時刻の一致判定に変えただけ)。
$RunHours = @(18)
$now = Get-Date
...
# 上記の$RunHours判定により、タスク自体は毎時起動のままでも1日1回(18:00)しか
# 実際にはバックアップ処理が走らない(それ以外の起動はスキップして即終了する)。その
# ためタスクスケジューラの設定は変更しなくても今回の削減目的は達成できる。
#
# もしタスク自体の起動回数(プロセス起動のオーバーヘッド)も減らしたい場合は、
# 以下の手順でトリガーを毎時起動→18:00の1日1回起動に変更できる:
#   1. タスクスケジューラ(taskschd.msc)を開く
#   2. タスクスケジューラ ライブラリから "KIC_Supabase_Backup" を探して右クリック→プロパティ
#   3. [トリガー]タブ→既存のトリガーを選択→[編集]
#   4. 開始時刻を18:00に変更し、「詳細設定」内の「繰り返し間隔」と「継続時間」を
#      両方とも無効化(チェックを外す)する(18:00に1回だけ実行される)
#   5. [OK]で保存

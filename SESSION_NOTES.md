...
## Step 3(着手前、次のアクション)
方針: 「agent_idを唯一の照合キーとし、文字列比較はagent_id不在時の正規化フォールバックに限定する」。対象は調査Dで洗い出したAgent関連5箇所:
1. 共通正規化関数を1つ新設(trim・小文字化・ピリオド/カンマ/連続スペース除去)、5箇所で共用
2. `resolveAgentIdForSave`: agent_id未決定時に無警告nullにせず、表記ゆれ候補提示または警告
3. `generateInvoice`の既存Invoice判定: agent_nameではなくbooking_id(+複数請求先時はagent_id)で判定
4. `showInvoicePreview`の住所取得・`salesItemBelongsToAgent`・残り1箇所(`resolveAgentIdForSave`呼び出し元の`saveBookingDetail`)も同方針
5. ホテル/レストラン/ガイド名のセッション内配列突合(hotel_name/restaurant_name/guide_name等)はスコープ外、変更しない

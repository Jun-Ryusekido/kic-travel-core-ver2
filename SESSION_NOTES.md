...
### ドライバー宿泊(2026-09-25 JUN決定: 案C。第1段階を実装、ブランチ claude/magical-ride-6phzj3、PR作成・マージはJUNの確認後)
...
- 第1段階の実装(index.html):
  - ホテルタブに「区分」列(ゲスト/ドライバー)。ドライバーの行はホテル名の上に紫の「DRV」、備考欄の案内文を「近くのバス駐車場など」に。
  ...
  - 仕入明細へ追加: ドライバーの行は「対象外」(disabled、ツールチップ・小さな注記「仮払いで精算」)。addArrRowToCostNow でも拒否。

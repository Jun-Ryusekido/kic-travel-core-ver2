...
【重要な前提】本日は${todayJstStr}です。カード利用日の性質上、この日付より未来の日付が取引日になることは絶対にありません。
...
- 【3つの数字が並んでいる場合（重要）】「ご利用日」欄（またはそれに準ずる日付欄）に、区切られた
  ...
  「月」）を読み飛ばして無視することは絶対にしてはならない（誤例:「8 6 12」を1つ目の「8」だけを
  月と誤認識して2026-08-12としてしまい、本来の月「6」が失われるのは誤り。正しくは
  [年8→2026][月6][日12]で2026-06-12）。
  - 3つの数字が見つかった場合の対応順序は、原則として[年(和暦想定)]→[月]→[日]とすること。
    ただし明細のヘッダーの表記や他の行・他の列の並び方から、明確に別の順序（例: 年が末尾にある等）
    であると判断できる場合は、その並びに従うこと。
  - この3数字ルールは、上記【和暦/西暦の判別】で説明している「年欄の『8』と月日欄の『5 21』を
    ...
    判断せず、必ず全ての数字の意味（年・月・日のどれに対応するか）を特定してから日付を組み立てる
    こと。
...
- 【和暦/西暦の判別（重要）】クレジットカード会社によっては、年の表記が西暦（2026、桁数4桁）ではなく
  ...
  - 【年・月・日は必ず独立した3つの数値として扱うこと（重要）】「令和8年2月19日」のような表記は、
    必ず「令和の年数=8」「月=2」「日=19」という3つの別々の数値として最初に抽出し、その後で
    「年」の数値(8)だけに変換式(+2018)を適用すること。西暦変換の計算に「月」や「日」の数字を
    混ぜたり、逆に「月」の値を「年」の数字から流用したりしてはならない（誤例:「令和8年2月19日」を
    年8→2026、月を2ではなく元号年数の8のまま流用して2026-08-19としてしまうのは重大な誤り。
    正しくは2026-02-19）。
  - 表のレイアウトによっては「年」の列と「月日」の列が別々のセル・別々の位置に分かれて印字されて
    ...
    年・月・日を正しく組み立てること
- 【出力前の未来日付セルフチェック（最重要・厳守）】"date"を確定させる前に、必ずそれが本日
  （${todayJstStr}）より未来の日付になっていないか確認すること。カード利用日が本日より未来に
  なることは業務上あり得ない。もし組み立てた日付が本日より未来になった場合、それは年・月の
  変換・抽出を誤った可能性が非常に高いという明確なサインである。落ち着いて年・月・日を数字ごとに
  読み直し、特に和暦変換で月の数字を年の計算に混入させていないかを再確認すること。再確認した
  結果、確信を持って未来にならない正しい日付が組み立てられた場合はそれを"date"に採用してよい。
  一方、再確認しても未来にしかならない場合や、正しい日付に確信が持てない場合は、絶対に未来の
  日付をそのまま"date"に入れてはならない。その場合は"date"をnullにし、"date_raw"に実際に
  印字されていた文字列をそのまま入れ、"needs_review"をtrueにすること。

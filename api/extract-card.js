...
// 仕入明細（請求書）読み取りプロンプト(text/pdf/image、計3箇所)で同一文言がコピペされていた
// ものを1箇所に集約したもの。sourceDescは読み取り元の説明文（例:「以下の請求書・仕入明細書」
// 「このPDF」「この請求書画像」）のみを呼び出し元ごとに差し替える。
//
// 【消費税対応(2026-08-31追加)】税抜明細のみ抽出され、消費税分が取り込まれない不具合
// (REF#876等)への対応。既存の「AIには計算をさせず、構造化出力のみ担当させる」原則に従い、
// tax_rate/amount_is_tax_included/tax_breakdown/subtotalは全て“転記”のみを求め、
// 税込⇔税抜の換算・消費税額の算出等の算術は一切AIにやらせない。税抜/税込の判定と
// 消費税行の生成は index.html の applyInvoiceTaxLines() がコード側で機械的に行う。
function buildInvoiceExtractionPrompt(sourceDesc) {
  return `${sourceDesc}から費用明細を抽出してJSON配列で返してください。
各費用項目を1つのオブジェクトとして配列に含めてください。
フィールド：supplier_name(請求書の発行元・支払先の会社名、不明は空文字), ref_no(ツアー番号・REF#・KICコード等、行またはブロックに記載があれば抽出、なければ空文字), expense_date(費用日付・YYYY-MM-DD形式、不明は空文字), content(費用内容・品目名), invoice_no(請求書番号・Invoice No.、記載があれば抽出、なければ空文字), category(区分：「D（ドライバー）」「ゲスト」「ゲスト・TG」「その他」のいずれか), unit_price(単価・数値・円、不明は0), qty(数量・数値、不明は1), amount(合計金額・数値・円、不明は0), tax_rate(この行に適用されている消費税率：8/10/0のいずれか。記載から判断できなければnull), amount_is_tax_included(amount欄が税込表記か税抜表記か。明細欄自体の記載から判断できる場合のみtrue/false、判断できなければnull), payment_method(支払方法：「現地払い」「前振込み」「後請求」「全旅クーポン」のいずれか、請求書の場合は「後請求」)
...
- 本体の運賃とは別に、発券手数料・保管料・送料等の付随費用が記載されている場合も、可能な限りそれぞれ個別のitemsの1要素として抽出してください（ただし消費税は下記tax_breakdownで扱うため、itemsの1行としては抽出しないでください）。
...
【消費税の取り扱い（重要・計算は絶対に行わないこと）】
- 請求書に消費税額が税率ごとに明記されている場合（例:「消費税(8%) ¥1,060」「内消費税10% ¥500」等）は、その税率と金額をそのままtax_breakdownに転記してください（例: [{"rate":8,"tax_amount":1060}]）。複数の税率が併記されていればtax_breakdownに複数要素を入れてください。消費税額の記載が請求書に見当たらない場合はtax_breakdownを空配列[]にしてください。
- subtotalには、請求書に「小計」「税抜合計」等として明記されている税抜金額があればその数字をそのまま入れ、記載が無ければnullにしてください。
- 【最重要】tax_rate・amount_is_tax_included・tax_breakdown・subtotalのいずれについても、あなた自身が金額を計算する（掛け算・足し算する、税込⇔税抜を換算する、消費税額を算出する等）ことは絶対に禁止します。請求書に実際に印字されている数字をそのまま転記するだけにしてください。税込/税抜の判定・消費税額の算出はこの後プログラム側で機械的に行います。
出力は配列ではなく、以下の形式のJSONオブジェクトとしてください：
{"items": [各費用項目の配列（フィールドは上記の通り）], "invoice_total_amount": 請求書に記載されている合計の振込金額・お支払い金額（数値、見当たらなければ0）, "tax_breakdown": [税率別消費税額の配列（上記の通り。記載が無ければ空配列）], "subtotal": 税抜小計（数値。記載が無ければnull）}
...
{"name":"氏名(日本語表記。姓と名の間に半角スペース、例:田中 太郎)","name_kana":"氏名のカタカナ読み(例:タナカ タロウ)","name_romaji":"氏名のローマ字表記(名刺に英語表記があれば記載通りに、無ければnull。例:TANAKA Taro)","phone":"携帯電話番号","birthday":"生年月日(YYYY-MM-DD形式。和暦のみの記載は西暦に変換して構いません。記載が無ければnull)","age":"年齢(数値。記載が無く生年月日からも算出できなければnull)","postal_code":"郵便番号(例:150-0001。記載が無ければnull)","address":"住所(記載が無ければnull)"}
...
JSONのみ返し、説明文・コードブロック記号は不要です。` }
    ...
    if (resolvedInvoiceText) {
      const data = await callClaude([{
        type: 'text',
        text: `${buildInvoiceExtractionPrompt('以下の請求書・仕入明細書')}
...
${resolvedInvoiceText}`
    ...
    if (invoicePdfBase64) {
      ...
      const data = await callClaude([
        ...
        { type: 'text', text: buildInvoiceExtractionPrompt('このPDF') }
      ], 8000);
      return res.status(200).json(data);
    }
    ...
    if (invoiceImageBase64) {
      const data = await callClaude([
        ...
        { type: 'text', text: buildInvoiceExtractionPrompt('この請求書画像') }
      ], 8000);
      return res.status(200).json(data);
    }

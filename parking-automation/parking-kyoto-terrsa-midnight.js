// ⚠️ 【使用禁止】2026年時点でreserva.beの利用規約（第4条禁止事項）を確認したところ、
// 「ロボット、クローラー、スパイダー及びこれらに類するプログラムを用いて、機械的な
// データ取得及び操作を行う行為」が明確に禁止されていることが判明したため、この
// スクリプトによる自動予約は中止した。Windowsタスクスケジューラの起動タスク
// （KIC_KyotoTerrsaMidnight）は無効化済み。今後は人間が毎晩0:00前後に手動で
// reserva.beにログインし、手動で予約操作を行う運用に切り替えている。
// このファイルは参考のため削除せず残しているが、絶対に再度スケジュール登録・
// 手動実行しないこと。
//

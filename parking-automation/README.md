...
- `--vehicles 2`（2026-08-28）の実機テストでは、1台目は成功しましたが2台目で「既に同じ日時で
  予約しています。さらに同じ日時に別予約を追加しますか。」という確認モーダルへの対応漏れにより
  失敗しました。モーダルの「予約を進める」を自動クリックする修正を加え、別日程（2026-08-29）で
  再検証する予定でしたが、実予約を伴うため確認が取れず、この時点では実機での再検証は行えて
  いません。**次回2台以上での実行時は、2台目以降が実際に確定するかどうかを結果ログで必ず
  確認してください。**

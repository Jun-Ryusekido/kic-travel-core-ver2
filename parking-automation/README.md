...
- `--dates 日付1,日付2,日付3`（カンマ区切り、優先順）で複数の候補日を指定できます
  （`--date`は1件だけの場合のエイリアスです）。**解禁待ちリトライ（2.5秒間隔・最大10分）を
  行うのは先頭の候補日のみ**です（本日深夜0:00に新規解禁される想定の日）。目標台数
  （既定2台）分の予約が埋まるまで、先頭の候補日から順に予約を試みます。ある候補日で
  満車等によりこれ以上予約できなくなった場合、2件目以降の候補日は解禁待ちをせず、
  その時点の空き状況を1回だけ確認してすぐ予約を試みます（早期に埋まりやすい人気日程を
  想定し、後続候補日での確認待ちで解禁の瞬間を逃さないため）。目標台数を確保できた
  時点でそれ以降の候補日には進みません。
- 2.5秒間隔・最大10分でカレンダーをリトライし、先頭の対象日が選択可能になり次第すぐに
  `--vehicles`（既定2台）分の予約を連続実行します。10分経っても解禁されない場合や
  候補日をすべて試しても目標台数に届かなかった場合は、その旨をエラーとして
  `logs/kyoto-terrsa-error-{日付}.log`に記録します（成功/失敗いずれの場合も

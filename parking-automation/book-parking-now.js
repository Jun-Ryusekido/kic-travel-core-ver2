...
// 名古屋(大型車両夜間宿泊予約システム)のどちらの自動化処理を使うか振り分ける。
//
...
// ではない）。config.json/nagoya-config.jsonにはそれぞれのサイトのログイン情報のみを保持する。
...
async function main() {
  ...
  console.log(`即時実行待ち: ${targets.length}件（新大阪: ${shinosakaTargets.length}件 / 名古屋: ${nagoyaTargets.length}件）`);
